...
⚠️ **第三個是隔壁的共用包**（2026-08-28 接 `winkit` 時加的）：它是 `pyproject.toml` 的 `[tool.uv.sources]` 以**相對路徑**相依進來的，而那種相依**看不見**——只把這一個資料夾傳給別人時 `uv sync` 會失敗，而它吐的是 uv 自己的路徑錯誤，說不出「你少複製了隔壁那個資料夾」。使用者換電腦是複製整個 `C:\SOURCE5\`，那時兩個都在；會缺的是「只複製了這一個」的情況。⚠️ **這一行顯示的是相對路徑、不是 `GetFileName()`**：它跟專案自己那個 `pyproject.toml` 同名，只印檔名的話訊息框上會並排兩個一模一樣的名字，使用者分不出少的是哪一個。⚠️ 訊息本身也多了一句括號（「上面若列出 ..\winkit，那是隔壁的共用資料夾，要跟專案一起複製」）——沒有那句的話，前一句「請把整個專案資料夾完整複製過來」照做也不會好。⚠️ 同一個理由，「安裝.bat」的 `uv sync` 失敗訊息也多列了這一條。
