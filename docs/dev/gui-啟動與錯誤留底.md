...
⚠️ **檢查的是 `target` 這個變數、不是另外手打一次檔名**，訊息裡的檔名也一律用 `fso.GetFileName()` 從被檢查的那條路徑取回來。手打的話，改掉 GUI 檔名時守門會安靜地繼續檢查舊名字。
...
使用者要求「以後共用包搬動目錄位置，只改一個地方就改好」。**唯一真值只能是 `pyproject.toml` 的 `[tool.uv.sources]`**——uv 只吃字面值，那一行非有不可，所以問題不是「要不要有一份」，而是**別處還抄了幾份**。接上共用包當天抄了三份：`.vbs` 的守門、`.vbs` 的 `EnvFresh`、`.bat` 的失敗提示。現在：

| 落點 | 作法 |
| --- | --- |
| `pyproject.toml` | 正本（`[tool.uv.sources]`） |
| `uv.lock` | uv 自己重寫，不必管 |
| 「啟動.vbs」守門 | 讀出來（`LocalSources`） |
| 「啟動.vbs」`EnvFresh` | 吃守門算好的**同一份**（多一個參數，不重讀） |
...
⚠️ **`.vbs` 那支剖析器刻意很笨**：逐行走，遇到 `[` 開頭就更新「現在在不在 `[tool.uv.sources]` 區段」，區段裡才找 `path` → 它後面的 `=` → 再後面的第一對引號。**非得看區段不可**（同一個 `pythonpath` 的坑，`.vbs` 版一樣會踩）；而「找引號」這種寫法不在乎空白怎麼排，也自然略過 `git = "..."` 那種不是相對路徑的來源。

⚠️ **不用 `ADODB.Stream`、也不用 `New RegExp`，兩個都是為了時間**：這一段在「雙擊到視窗出現」的路上（整條 540 ms，下面 1b 那條快速路徑省的也才 35～40 ms），而這兩個物件光是**建起來**就各要幾毫秒。實測（同一台機器、200 次取平均）：FSO 讀檔 + `InStr` 掃描 **1.78 ms**、換成 `RegExp` **3.71 ms**、只讀不掃 1.82 ms —— 也就是說掃描本身 ~0，成本全在讀檔那一次，而那個檔早在 `EnvFresh` 比對時間戳時就被作業系統快取了。⚠️ 用 FSO 讀等於用**系統編碼**（cp950）解一份 UTF-8 檔，中文註解會變亂碼——**不影響**：要抓的兩行（區段標頭、`path` 那一行）純 ASCII，而 Big5 的次位元組範圍不含換行字元，行不會被吃掉。下面 `Captured()` 攔 traceback 那支仍然走 `ADODB.Stream` 的 UTF-8：那裡要的正是中文讀得出來，而且它已經在失敗路徑上，幾毫秒不重要。

⚠️ **失效方向：守門是誤報，剖析器是漏報**。抓錯路徑會讓**每一次正常啟動**都被擋（誤報），所以剖析器寧可放手也不猜——看不懂的寫法就當作沒有這條相依，代價只是守門少檢查一項、使用者退回去看 uv 的英文錯誤（漏報）。但**漏報是沉默的**，所以 `tests/test_docs.py` 用三支釘住：`[tool.uv.sources]` 列的路徑要真的存在（誤報）、兩個入口檔裡不可以有第二份字面值（搬完家還在檢查舊路徑）、以及**把 `LocalSources` 原封不動抽出來餵給 `cscript`**，拿它抓到的東西跟 `tomllib` 對答案（漏報）。⚠️ 最後那一支刻意跑**出貨的那一份**、而不是在測試裡照它的規則再寫一次——再寫一次就是兩份會各自漂的程式。故障注入驗過三種：把來源路徑改成不存在的、在 `.vbs` 裡塞回一份字面值、把來源改寫成 `[tool.uv.sources.winkit]` 多行表（`tomllib` 讀得到、`.vbs` 讀不到），三種都紅。
...
- **事前 `EnvFresh()`**：`.venv\Lib\site-packages` 的修改時間必須不比 `pyproject.toml`、`uv.lock`、隔壁共用包的 `pyproject.toml` 舊。任何一項對不上就整個走 `uv`，讓它去補。⚠️ 比的是**那個資料夾**、不是 `pyvenv.cfg`——後者只在建立環境那一次寫，之後 `uv sync` 裝了什麼、拿掉什麼都不會動到它，拿它當時間戳等於永遠回答「環境是新的」。⚠️ 共用包只看它的 `pyproject.toml`（相依變了才要重裝）；它的**原始碼**改了不必，那是 editable 安裝，`import` 直接讀那個資料夾。⚠️ **共用包在哪裡不寫死在這裡**：與上面那道守門吃同一份 `LocalSources` 的結果（多收一個參數，不自己重讀一次檔）。⚠️ **這一道退成 `False` 之後不會自己好**（2026-08-28 改上面那條規則時當場撞到）：只改 `pyproject.toml` 的**註解**也會讓它從此回 `False`，而 `uv sync` 認定沒事可做時**不會回頭去動 `site-packages`**——於是快速路徑安靜地退休，徵狀只有「好像有點慢」。改完 `pyproject.toml` 記得跑一次 `uv sync`；它印的是 `Checked N packages`（沒真的裝東西）的話，把 `site-packages` 的時間戳碰一下（`os.utime`，那正是真的裝了東西時會發生的事）。
