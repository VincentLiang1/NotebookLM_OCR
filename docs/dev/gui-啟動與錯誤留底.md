...
`App.__init__` 在建介面**之前**開好這一趟的紀錄檔（`logs` 底下、檔名是啟動時間），順序不能反：建介面途中炸掉的話，那是唯一收得到的地方。作法照 `meeting-scribe`（使用者指定），四件事直接沿用：

- **一次執行一個檔**：一次啟動正好等於一個檔，不必另外定義邊界，跨午夜也不會被切成兩半。
...
- `logs\<啟動時間>.log` —— GUI 每次啟動一個檔，保留 30 天，已列進 `.gitignore`（`logs/` 與 `*.log` 兩條都有）。
