...
- **一次執行一個檔**：一次啟動正好等於一個檔，不必另外定義邊界，跨午夜也不會被切成兩半。⚠️ **檔名要帶 pid 才真的成立**：兩個行程可能在同一秒走到 `open_run_log`，兩邊都是 `open("a")`，同一個檔被兩份輸出交錯寫進去——而那正是最難讀懂的一種紀錄，偏偏出事時要讀的就是它。

  ⚠️ **這條的理由 2026-08-29 訂正過，而且訂正之後它更站得住。** 原本寫的是「雙擊兩次『啟動.vbs』」，那條路在 2026-08-28 加了單一實例把關之後就被擋掉了（那一輪漏改這一句，於是這裡留著一個已經不成立的理由——**規則還對，但它的理由過期了**，而過期的理由下次會被當成「已經不需要」而拿掉）。真正的理由是 `winui.single_instance_or_raise()` 的**第三格**：鎖被拿走、卻**找不到**那個視窗時照常開（失效方向往放行倒，共用包自己的說法是「這是便利、不是安全邊界」）。那一格到得了——第一個實例還在 `App.__init__`（視窗全程 `withdraw()`、標題比對不到）、卡在某個訊息框、正在 `destroy()` 與行程結束之間，或者是個已經沒有視窗的殘留行程（onnxruntime 跑在 daemon thread 上，直譯器關不掉時就是這一種）。⚠️ 而且紀錄檔是 `App()` 才開的，也就是**通過那道把關之後**——把關與這個檔名不在同一條防線上，所以「有把關了所以可以拿掉 pid」是錯的。

  這一輪的來源是 meeting-scribe 那個 session（2026-08-29）：它把這個檔名形狀抄過去時查出**自己那邊更容易踩到**（網頁版沒有單一實例把關，自動從 7860 找空埠，兩份本來就跑得起來，而兩次啟動各付 2.6 秒的 Python 起步，連點兩下常常落在同一秒），順帶把「原生視窗有把關也擋不住」這一點講清楚了。⚠️ 共用包的 `purge_old()` 對帶 pid 的檔名照樣有效：它掃 `*.log` 看 mtime，不看檔名格式。
...
⚠️ **這四件 2026-09-19 起不再是自己實作的，改用共用包的 `filelog.Writer`**（原本這裡有一份完整的手抄複製品，`log_header()` / `open_run_log()` / `_log_write()` / `_log_close()` 四支，連「逐行 flush：使用者是直接關視窗收工的」那句註解都一樣照抄）。換過去同時買到第五件：

- **開了沒轉東西的那一趟不留檔**（延後開檔）：檔案等到第一次真的有內容才建立，六行檔頭在那一刻先補上。⚠️ 換之前的實測：本機 `logs` 裡 307 份主檔有 **253 份（82%）**是只有檔頭的空殼——視窗開著不一定在做事（調個選項、看一眼上次的結果、拖錯檔又關掉），而它們還會**害人挑錯檔**：「排序最後的就是最新的」是找上一趟紀錄最自然的挑法，最新那幾份往往正是空殼。⚠️ **不能改成「收尾時把沒內容的刪掉」**：使用者是直接關視窗收工的，那條路根本不會跑（與逐次 flush 同一個前提）。
  - ⚠️ **開場白與收尾行要走 `preamble()` / `footer()`，不可以走 `raw()`**：那三處（`__init__` 最後那行「執行紀錄：<檔名>」、建日誌區時那句首次下載模型的提示、`_log_close` 的「---- 結束 ----」）用 `raw()` 就會把檔案生出來，於是延後在**最常走的那條路**上等於沒有——空殼的長相正好就是六行檔頭＋兩行開場白＋一行收尾。`_append(..., opening=True)` 是它在這支程式裡的入口。
  - ⚠️ **延後的是檔案、不是 `logs` 資料夾**（建構子照舊 mkdir）：那個資料夾同時是「紀錄檔還沒落地時，『開啟紀錄』那顆鈕退到哪裡」的落點。
  - ⚠️ **「開啟紀錄」那顆鈕因此多一條退路**：`_open_log` 在檔案還不存在時開那個資料夾而不是那個檔，否則一開 App 就按下去會**毫無反應**（`os.startfile` 對不存在的路徑丟例外，而它被吞成一行日誌）。那顆鈕變灰的判準沒變（只看 `_log_path is None`）——「等一下就會有」不該讓它先灰掉。
