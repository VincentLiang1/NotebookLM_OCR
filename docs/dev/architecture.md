...
⚠️ **2026-08-28 建起來了，而且只有一包**（使用者裁定）。原本規劃兩包（Windows 專用 ＋ 跨平台底層工具），前提是 meeting-scribe 也要接；⚠️ **meeting-scribe 當時不接**（它正往 macOS／iOS 走，共用到 mac 太複雜），所以那個前提不成立，整包就是 Windows 專用、不必為跨平台留餘地。⚠️ **2026-08-29 那個「不接」翻案了**（使用者當天決定會議逐字稿那一支也要接），但**還沒接線**——整包仍然是 Windows 專用，因為「不必為跨平台留餘地」的理由已經換成「這包本來就只做 Windows 整合」，不是靠那一支的去留撐著。它維持自己那一份路徑模組（本包的路徑模組當初正是從它抄來的），**不要回頭去改它**。

下游**兩支已接線**：本專案與 `C:\SOURCE5\Python\MP4-2-SRT`。⚠️ **2026-08-29 起帳面上是三支**（多了會議逐字稿那一支），但它**還沒接線、也還不在 `check_downstreams` 的清單裡**——所以改共用包時實際要顧的影響面暫時仍是兩支。⚠️ 兩個數字要分開講：**接了幾支**決定改壞了誰會紅，**帳面幾支**決定這個 API 以後要養多久。
...
⚠️ **改完要三個 repo 同一輪一起改完、一起提交**（使用者 2026-08-28 定）。它們是不同的 git repo、湊不成同一筆 commit，所以「一起」指的是**同一輪動作**：全部改完、測試全綠，才一次提交完。⚠️ 理由是**只提交共用包那半等於沒有修好**——加一支函式而下游不呼叫，使用者那邊什麼都不會變。2026-08-28 的 `winui.place_window` 就是這樣栽的：共用包那筆提交完、下游還沒接，使用者去測，回報「還是會啟動在不同的位置」。⚠️ 所以也**不再生 PROMPT 讓姊妹專案自己改**（同日改定，原本是那樣）：連帶要動的下游檔案同一輪一起改完。完整的七步在共用包自己的 `CLAUDE.md`（⚠️ 那一份不會自動載入，動它之前先讀）。

⚠️ **`winkit.bind()` 的呼叫點放在 `pdf2ppt/__init__.py`，是因為任何子模組被 import 都會先經過它**——放在別處就會有一條「先 import 了某個子模組、而綁定還沒發生」的路徑，症狀是 `host()` 當場丟例外，或更糟：落地位置算在還沒綁定的預設值上。

⚠️ **動共用包之前先 `ListAgents`，而且有別的 session 在跑時提交一律逐檔指定路徑**（不准 `git add -A`）——理由、實例與災情見 `docs/dev/collaboration.md` §13。

⚠️ **這一段的前提 2026-08-29 換過**：`CLAUDE.md` 原本那條「不要自動跨 repo 改、一律生 PROMPT」已經改成**三步判定**（工作樹乾不乾淨 → `ListAgents` → 把那個 repo 的 `CLAUDE.md` 完整讀過），判定過了就直接改。所以「共用包沒有平行 session、可以直接改」不再是它的**例外**，而是同一條判定跑出來的結果——⚠️ **共用包那一格仍然要跑判定**，「它沒有平行 session」這個 2026-08-28 的觀察在 2026-08-28 當天就被推翻過（同時有兩個在跑，見 `docs/dev/collaboration.md` §13）。
