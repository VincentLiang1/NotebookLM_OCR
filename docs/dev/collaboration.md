...
使用者指示「CLAUDE.md 裡面可以搬移的內容，移到 `docs/dev` 去」，同一輪還要求「比照 `MP4-2-SRT` 的 `docs` 產出系統規格與 spec 目錄」。兩件事其實是同一件：**那 78 條不變量本來就是規格書的內容，只是住錯地方。**
...
**釘法**：`tests/test_docs.py` 從 8 條擴到 12 條——常數值與符號指路改成掃 `CLAUDE.md` ∪ `docs/系統規格.md` ∪ `docs/spec/*.md`（規則搬家之後任何一份漂掉都一樣害人），另加章號連號、H1 與檔號一致、`00` 的文件地圖與目錄雙向一致、CLAUDE.md 指得到每一章。⚠️ 最後這條是整個安排的支點：**CLAUDE.md 是唯一自動載入的檔，它沒指到的章節等於不存在。**
