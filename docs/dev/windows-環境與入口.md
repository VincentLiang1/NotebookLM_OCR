...
圖形是**藍色圓角磚 + 一張 16:9 的白色投影片（橘色標題列）+ 投影片後面升起的三道同心弧**。弧取自 NotebookLM 標誌的語彙（使用者 2026-08-25 提供 `notebook-logo.svg` 當參考），投影片是本工具的產物。

⚠️ **不可直接沿用 NotebookLM 的商標圖形**。原稿是三道**共用左緣、半徑遞減**的拱，色票 `#3186FF`／`#4FA0FF`／`#76BBFF`→`#A9A8FF` 徑向漸層。我們刻意改成**對稱同心**、換掉色票（磚底 `#2B7CF6`）、並把它放進「投影片」這個本專案自己的主體裡——取語彙，不取那個標誌。

設計過程量到的三件事，改圖形之前要知道：

- ⚠️ **卡片必須比弧寬，弧腳要整個藏在卡片後面**。弧腳只要露在卡片兩側，整張圖就會讀成一把**掛鎖**——當天排了 10 個變體目視，露腳的 5 個全中（含使用者一度選中、看到縮圖後自己否掉的那一版）。
- ⚠️ **卡片外圍要留一圈磚色的縫**（`HALO`=18）。白弧碰到白卡片會糊成同一塊剪影，加了縫才分得開。
- ⚠️ **16–32 px 另有一版簡化圖形**（`icon-small.svg`：單弧、筆畫 58、卡片只留標題列）。滿版那版的弧縫是 14/512，換算下來 32 px 只剩 0.9 px，三道弧會糊成一片雜訊。兩版並排放大目視比過，分界就在 32 px（`SMALL_MAX`），`.ico` 裡 ≤32 的四個尺寸一律取簡化版。
