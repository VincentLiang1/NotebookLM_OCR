...
- **容器統一成卡片**。原本三種視覺重量同時在場：「檔案」是 `LabelFrame`（蝕刻邊框＋標題）、選項是 `Card.TFrame`、收合鈕是扁平長條。「檔案」改成 `Card.TFrame` 之後，那個標題也不必了——卡片裡第一行就寫著「輸入 PDF」。⚠️ 動作列**不要**也包成卡片：主要動作要浮在版面上，不是再被裝進一個盒子。
...
**改完量到的 `reqheight`（邏輯 px）**：閒置 426、選項展開 655、日誌展開 610。⚠️ **手風琴仍然成立**：兩區同時攤開是 **839**，而 1080p@125% 的筆電工作區只有約 810——比舊版的 953 好很多，但還是超。這個數字要留著，下一個想拆手風琴的人才有東西可以比。
