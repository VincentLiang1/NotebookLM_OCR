...
`_set_advanced()` / `_set_log_shown()` 互斥：展開進階就收日誌、按下轉檔就收進階並展開日誌。實測三個狀態的 `reqheight`（實體 px @150%）：閒置 462、進階展開 1128、執行中 746，工作區 1368 —— 全部塞得進去。高度一律由 `_fit_window()` 決定（見 §5 第 3 點）。
