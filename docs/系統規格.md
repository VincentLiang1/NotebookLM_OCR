...
**`--cover` / `--no-cover` 只決定「色塊畫在哪」**，不是精緻度開關。`--no-cover` 讓底色長在文字方塊上（在 PowerPoint 裡移動文字時底色跟著走），色帶因此比字墨高 0.20em（PowerPoint 的行首 leading 補償）——⚠️ 裁切必須知道這件事，而分段蓋板、弧形切線段、同列/堆疊裁切**兩種模式都要做**。
