...
# `new_paths()` 不帶 pid，而兩個行程並存在這支程式裡是**到得了的狀態**（見
# `open_run_log`）。⚠️ 2026-08-29 訂正：這段原本寫的理由是「雙擊兩次『啟動.vbs』」與
# 「姊妹專案一次只會有一個行程」，兩句在今天都不成立了——前者在 2026-08-28 加了單一
# 實例把關之後就被擋掉（那一輪漏改這裡），後者是 meeting-scribe 當天查出來的：它的
# 網頁版根本沒有那道把關（自動找空埠，兩份本來就跑得起來），比這邊更容易踩到。
...
def open_run_log() -> tuple[Path | None, "io.TextIOBase | None"]:
    ...
    # ⚠️ **單一實例那道把關擋不住這件事**（2026-08-29 訂正；這段原本寫的理由是「雙擊
    # 兩次『啟動.vbs』」，那條路 2026-08-28 起確實被擋掉了）：`single_instance_or_raise()`
    # 的第三格是「鎖被拿走、卻**找不到**那個視窗就照常開」（失效方向往放行倒，共用包
    # 自己的說法是「這是便利、不是安全邊界」），而那一格到得了——第一個實例還在
    # `__init__`（視窗全程 withdraw、標題比對不到）、卡在某個訊息框、正在 destroy 與
    # 行程結束之間，或者是個已經沒有視窗的殘留行程（onnxruntime 跑在 daemon thread
    # 上，直譯器關不掉時就是這一種）。⚠️ 而且紀錄檔是 `App()` 才開的，也就是**通過那
...
def main() -> int:
    ...
    # ⚠️ **已經開著就不要再開第二個**（使用者 2026-08-28：「啟動多個程式沒有用」）。
    # 兩份轉檔會搶同一顆 GPU、也各自載一份 OCR 模型，而畫面上兩邊都以為自己在正常
    # 轉檔——那是一種查不出來的慢。整條政策（含「找不到那個視窗時照常開」）在
    # `winui.single_instance_or_raise`，這裡不再手抄一份。
    # ⚠️ **排在 `is_project_dir()` 之後**（2026-08-28 更正）：跑得起來的副本才有
    # 資格佔住那把鎖。擋在最前面的話，一份缺了 `pdf2ppt/` 的副本只要把
    # `fail_no_project()` 的框留在螢幕上，就會一直持鎖，而它的視窗（class `#32770`
    # 的訊息框、標題 `tk` 的隱藏 root）**永遠比對不到** `APP_TITLE`——於是另一份
    # 好的副本再也啟動不了。⚠️ 原本擋在最前面的唯一理由是「不要多一個只有檔頭的
    # `logs\` 檔」，而那個理由在這個位置仍然成立：紀錄檔是 `App()` 開的。
    # ⚠️ 離開碼是 **0**：啟動器因此不跳訊息框，也不會觸發它那道「非正常結束又不到
    # 5 秒就用 uv 再跑一次」的退路（那會真的開出第二個視窗）。
