...
def main() -> int:
    ...
    # 轉檔——那是一種查不出來的慢。找得到既有的就把它叫到前景：什麼都不做的話，
    # 使用者會以為程式壞了、再點兩三次。
    # ⚠️ **擋在最前面**：紀錄檔還沒開、視窗還沒建，退出時什麼都不必收（第二個實例
    # 若走到 `App()`，`logs\` 底下就會多長一個只有檔頭的檔）。
    # ⚠️ 離開碼是 **0**：啟動器因此不跳訊息框，也不會觸發它那道「非正常結束又不到
    # 5 秒就用 uv 再跑一次」的退路（那會真的開出第二個視窗）。
    if not winui.claim_single_instance():
        winui.raise_existing_window()
        return 0
