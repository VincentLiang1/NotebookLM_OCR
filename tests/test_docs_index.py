...
def test_claude_md_stays_small():
    ...
    MP4-2-SRT 是 18,000)。本 repo 定在 28,000,理由是它的主體與那兩個不同:

    2026-08-23 已經做過一次抽離(使用者指示),81 條規則的完整理由、門檻與反例
    全搬進 `docs/spec/`,這裡只留**規則本身、關鍵門檻與程式指路**。剩下的六段
    「不變量索引」合計約 15,168 字元、佔全檔 58%,平均 **187 字元/條**——那已經
    是「規則+門檻+⚠️陷阱」的最小形。要壓進 18,000 只剩 91 字元/條,等於砍掉
    門檻數字與陷阱警告,而**那兩樣正是它每次載入的理由**:這些是不變量,改任何
    一處都不得違反,不像領域文件可以「動到才讀」。

    ⚠️ **放寬不等於不守**:六段以外的部分只有 10,943 字元,新增的規則一樣要先問
    「這條沒每次載入會不會做錯」——會做錯才留在這裡,否則進 `docs/spec/` 對應章。
    判準沒有變:長篇內容放 `docs/spec/` 與 `docs/dev/`,這裡只留摘要、指路,
    以及「**用到時才知道就來不及**」的那幾條。分層規範見 `docs/dev/documentation.md`。"""
