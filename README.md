...
- **傾斜文字**：偵測器給出旋轉框時直接輸出旋轉文字方塊；弧形緞帶這類偵測失敗的情況用多角度重辨識救援
...
建議安裝 [Noto Sans TC](https://fonts.google.com/noto/specimen/Noto+Sans+TC) 字型（預設輸出字型，可用 `--font` 改成其他字型）。
...
詳細的演算法決策與校準紀錄見 [CLAUDE.md](CLAUDE.md)。
...
在 15 頁的範例簡報上，文字召回率約 97.5%（其餘為抓得比參考版更完整、或繁簡字形平手的情況）。
