...
> **黑視窗與錯誤訊息**：`啟動.vbs` 把主控台隱藏起來，所以原本會印在那裡的東西（`uv` 的錯誤、Python 的 traceback、GUI 自己的未預期錯誤）全部改寫進同資料夾的 **`啟動.log`**，每次啟動覆寫。程式若沒能正常結束，`.vbs` 會跳訊息框並問你要不要直接開啟那份 log。轉檔過程的訊息照舊顯示在介面下方的日誌區。
