...
**主畫面只留兩樣東西**：輸入 PDF 與輸出 PPTX，底下就是藍色的「▶ 開始轉檔」。其餘選項（頁碼、中文字型、推論裝置、粗體模式、渲染 DPI、最低信心分數、辨識語言，以及全部的進階開關）收在**開始轉檔底下**那顆「▸ 進階選項」按鈕裡，按一下才展開 —— 它們都有校準過的預設值，日常轉檔一項都不必動。展開時視窗會自己長高、收合時還原成原本的高度。GUI 的預設值與命令列完全一致。
...
離開碼有三種：**`0`** 全部正常、**`3`** 有頁面降級（檔案可用，降級的頁碼列在最後一行 `WARNING:`）、**`1`** 每一頁都失敗。圖形介面會把這三種結果分別顯示出來。
