...
| `--cover` | 啟用精修色塊機制（同列／堆疊裁切、雙色橫幅、弧形色塊等）。**預設關閉**：每個文字方塊以原色塊顏色自帶底色蓋住點陣文字、版面較簡單，移動文字時底色跟著走 |
...
- 預設每個文字方塊以原色塊顏色自帶底色（不另加獨立色塊，移動文字時底色跟著走）；漸層或照片背景上估不出單一底色的行則維持透明疊圖。需要精修色塊（裁切／雙色橫幅／弧形）時加 `--cover`
