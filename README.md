...
| **啟動（顯示訊息）.bat** | 同上，但保留主控台視窗顯示訊息 —— 啟動不起來、要看錯誤時用 |
...
>
> 紀錄檔萬一寫不出來（磁碟唯讀、防毒攔截），改用 `啟動（顯示訊息）.bat`：它保留主控台，訊息直接顯示在視窗裡。
...
離開碼有三種：**`0`** 全部正常、**`3`** 有頁面降級（檔案可用，降級的頁碼列在最後一行 `WARNING:`）、**`1`** 每一頁都失敗。拖放用的「轉檔.bat」與圖形介面都會分別顯示這三種結果。
